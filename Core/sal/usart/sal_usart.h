...
namespace sal
{
    ...
    {
        TX_ERROR,       // 发送失败,一般为非FIFO模式下上一次发送未结束;也有可能是内存泄露导致返回值被修改
        BLOCK_FINISH,   // 阻塞发送
        BLOCK_TIMEOUT,  // 阻塞发送超时
        TX_ONGOING,     // dma/it发送中
        TX_WAITING,     // dma/it已经加入发送队列等待
        BUFF_FULL,      // 发送缓冲区已满
    };
    /* 发送模式枚举 */
    enum class UartTxType
    {
        DMA,
        IT,
        BLOCK,
    ...
    /* 接收模式枚举 */
    enum class UartRxType
    {
        BLOCK_NUM,  // 阻塞接收,注意无法使用回调
        DMA_NUM,    // DMA接收一定字节后进入回调
        IT_NUM,     // IT接收一定字节后进入回调
        BLOCK_IDLE, // 阻塞+空闲,接收一定字节或者触发空闲后返回,无法使用回调
        DMA_IDLE,   // DMA+Idle,接收一定字节或者触发空闲时进入回调
        IT_IDLE,    // IT+Idle,接收一定字节或者触发空闲时进入回调
    };
    ...
    public:
        ...
        struct UartConfig
        {
            ...
            uint16_t rx_size = 0;
            UartRxType rx_type = UartRxType::IT_IDLE;
            UartRecvCallback rx_cbk = nullptr;

            bool use_fifo = false;
            uint8_t queue_mx_size = 1;
            UartTxType tx_type = UartTxType::IT;
    ...
    private:
        ...
        LoopQueue<uint8_t *> tx_queue_; // 注意此处只保存指针,module需自行维护数据块
        LoopQueue<uint16_t> tx_len_queue_;
        UartTxType tx_type_;
    ...
    public:
         ...
         *
         * @param type UartTxType的枚举值,分别为DMA/IT/BLOCK
         * @param use_fifo 是否使用发送队列,默认为false
         * @param queue_mx_size 发送队列最大长度,默认为0不使用
         */
        void UartSetSendType(UartTxType type, bool use_fifo = false, uint8_t queue_mx_size = 0);
         ...
         *
         * @param type UartRxType的枚举值,分别为分别为 DMA/IT/BLOCK + NUM/IDLE
         * @param size 期望接收一包的数据量,以byte计
         */
        void UartSetRecvType(UartRxType type, uint16_t size);
